...
import car.rental.app.domain.saga.event.PaymentConfirmedEvent;
...
public class ReservationSagaListener {
    ...
    public void onPaymentCompleted(String json) {
        log.info(() -> "Received PaymentCompleted JSON: %s".formatted(json));
        try {
            PaymentCompletedEvent event = objectMapper.readValue(json, PaymentCompletedEvent.class);
            log.info(() -> "Received PaymentCompletedEvent: %s".formatted(event));
            ...
            log.info(() -> "Created PaymentConfirmedEvent: %s".formatted(confirmedEvent));
